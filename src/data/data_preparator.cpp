...
DataPreparator::DataPreparator(const std::string& data_root_path, int random_seed)
    :
    base_path(data_root_path), current_train_index(0),
    X_train(0, 0),
    X_test(0, 0),
    y_train_one_hot(0, 0),
    y_train(0, 0),
...
Matrix DataPreparator::load_vectors(const std::string& filename, int num_rows, int num_cols, bool verbose){
...
Matrix DataPreparator::load_labels(const std::string& filename, int num_rows, bool as_one_hot, int num_classes) {
    ...
    for (int i = 0; i < num_rows; ++i) {
        if (!std::getline(file, line)) {
            throw std::runtime_error("File error: Not enough rows. Expected " + std::to_string(num_rows) + " Got: " + std::to_string(i));
        }
        std::stringstream ss(line);
...
void DataPreparator::load_data() {
    ...
    X_train = load_vectors(base_path + "fashion_mnist_train_vectors.csv", 60000, 28*28, false);
    ...
    X_test = load_vectors(base_path + "fashion_mnist_test_vectors.csv", 10000, 28*28, false);
...
void DataPreparator::standardize_data() {
    ...
    std::cout << "Test data standardized" << std::endl << "All data standardized" << std::endl;
}
